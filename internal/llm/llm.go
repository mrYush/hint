...
func AskLLM(cfg *config.Config, ctx *context.DirectoryContext, question string) (string, error) {
	// Resolve the provider profile; the loader has already validated it
	profile, err := cfg.Default()
	if err != nil {
		return "", err
	}
	...
	// Sending the request
	url := fmt.Sprintf("%s/chat/completions", strings.TrimRight(profile.BaseURL, "/"))
	...
	// Keyless profiles (e.g. local Ollama) must not send an empty Bearer
	if profile.APIKey != "" {
